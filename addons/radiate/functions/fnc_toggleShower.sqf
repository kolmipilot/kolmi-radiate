...
/*
 * Author: kolmipilot
 * Handles on and off decon shower
 ...
 *
 * Example:
 * [this, true] call kolmir_radiate_fnc_toggleShower;
 * Public: No
 */
...
if (_on) then {
    ...
    _trg setTriggerStatements ["this", "ace_player call kolmir_radiate_fnc_decontamination;", ""];
    _particles pushBack _trg;

    // Update objects list to delete
    _shower setVariable [QGVAR(cbrn_objects),[_sound] + _particles];

    if (isNull (_shower getVariable [QGVAR(cbrn_puddle), objNull])) then {
        ...
        _shower setVariable [QGVAR(cbrn_puddle), _waterPuddle];
...
} else {
    {
        ...
        detach _x;
        deleteVehicle _x;
    } forEach (_shower getVariable [QGVAR(cbrn_objects),[]]);
